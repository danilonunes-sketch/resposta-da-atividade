...
typedef enum {
    ITEM_ARMA,
    ITEM_POCAO,
    ITEM_ESCUDO, // Adicionado tipo de item escudo 
} TipoItem;
...
typedef struct{
    int   absorcao;
} DadosEscudo; // struct para o escudo feita por mim 
...
typedef struct {
    ...
    int     vida;
    float   dano;
    int     armadura; // Adicionado campo armadura
} Jogador;
    ...
    for (int i = 0; i < quantidade; i++) {
        ...
        if (it->tipo == ITEM_ARMA) {
            ...
            it->dados.arma.alcance = GetRandomValue(1, 3);
             ...
             if(GetRandomValue(0, 9) < 3) {  // 30% de chance de gerar uma poção
                it->dados.pocao.cura = GetRandomValue(10, 30);
            it->dados.pocao.cura = -20; // Valor negativo para indicar que é uma poção de dano
            } else {
                it->dados.pocao.cura = GetRandomValue(10, 30);// Valor positivo para indicar que é uma poção de cura
            }
...
void aplicarItem(Jogador *j, Item *item) {
    switch (item->tipo) {
        ...
        case ITEM_ESCUDO:
            ...
            break; // Adicionado case para o escudo
    }
    item->coletado = true;
}
...
void desenharItem(Item *item) {
    ...
    switch (item->tipo) {
        ...
        case ITEM_ESCUDO: 
        cor = DARKGRAY;  
        break; // Adicionado cor para o escudo
    }
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        BeginDrawing();
            ...
            DrawText("Vermelho = arma (aumenta dano) | Verde = pocao (cura) | Azul = escudo (aumenta armadura)", 10, 34, 18, GRAY);
