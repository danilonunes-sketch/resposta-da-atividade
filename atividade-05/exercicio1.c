...
void ordenarPorDistancia(void) { // ordena o vetor de ponteiros de entidades pelo inimigo mais próximo do jogador
    ...
    if (indiceMaisProximo != 1) {  // se o inimigo mais próximo não estiver na posição 1, troca os ponteiros
        Entidade *tmp = vetorEntidades[1];
        vetorEntidades[1] = vetorEntidades[indiceMaisProximo];
        vetorEntidades[indiceMaisProximo] = tmp; // troca de ponteiros no vetor, sem mover structs
    }
}
