...
typedef struct {
    ...
    int       valor;
    bool      coletada;
    float    tempoColeta;   // opcional: para animação de desaparecimento
} Moeda;
...
Color corDaMoeda(TipoMoeda tipo) {
    switch (tipo) {
        ...
        case MOEDA_DIAMANTE: return (Color){0, 0, 255, 255}; // azul para diamante
...
int valorDaMoeda(TipoMoeda tipo) {
    switch (tipo) {
        ...
        case MOEDA_DIAMANTE: return 50; // valor para diamante
...
TipoMoeda SortearTipoMoeda(void) {
    ...
    if (chance == 0) {
        return MOEDA_DIAMANTE; // 10% de chance
    } else if (chance >= 1 && chance <= 3) {
        return MOEDA_OURO;     // 30% de chance
    } else if (chance >= 4 && chance <= 6) {
        return MOEDA_PRATA;    // 30% de chance
    } else {
        return MOEDA_BRONZE;   // 30% de chance
    }
}
...
Moeda *criarMoedas(int quantidade) {
    ...
    for (int i = 0; i < quantidade; i++) {
        ...
        m->raio     = 10.0f;
        m->tipo     = SortearTipoMoeda(); // sorteia o tipo da moeda com base nas probabilidades definidas
...
bool tentarColetar(Moeda *m, Vector2 posJogador, float raioJogador) {
    ...
    if (distancia <= somaRaios) {
        m->coletada = true;
        m->tempoColeta = GetTime(); // opcional: registrar o tempo da coleta para animação
        return true;
    }
    return false;
}
    ...
    while (!WindowShouldClose()) {
        ...
        for (int i = 0; i < TOTAL_MOEDAS; i++) {
            ...
            if (m->coletada && (GetTime() - m->tempoColeta >= 3.0f)) {
                m->pos = (Vector2){ GetRandomValue(30, LARGURA_JANELA - 30),
                                     GetRandomValue(30, ALTURA_JANELA - 30) };
                m->coletada = false; // opcional: animação de desaparecimento
                m->tipo = SortearTipoMoeda(); // sorteia o tipo da moeda com base nas probabilidades definidas
                m->valor = valorDaMoeda(m->tipo);
            }
